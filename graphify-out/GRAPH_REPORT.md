...
## Communities (306 total, 76 thin omitted)
...
Cohesion: 0.06
Nodes (30): ODSExecutor, Sort records by identity fields to prevent deadlocks.                  When mult, Core executor for PostgreSQL ODS operations.          This class provides method, Fetch all column names for the target table., Validate records for JSON serialization safety.                  Checks:, Fetch column names and their PostgreSQL data types., Align record keys with actual table columns:         - Filters out keys that do, Validate that the target schema and table exist.                  Args: (+22 more)
...
Cohesion: 0.09
Nodes (34): create_data_source(), DataSourceCreate, DataSourceResponse, DataSourceUpdate, _decrypt_data_source_password(), delete_data_source(), _encode_config(), _encrypt_data_source_password() (+26 more)
...
Cohesion: 0.12
Nodes (26): EmailExecutor, Initialize EmailExecutor with Jinja2 SandboxedEnvironment.                  Crea, Core executor for email sending operations.          This class provides secure, EmailConfig, EmailContent, SMTP configuration for email sending.          This model holds all SMTP connect, Email content with subject and body templates.          Attributes:         subj, Tests for email_executor.py - Email execution service.  TDD RED phase: Tests sho (+18 more)
...
Cohesion: 0.16
Nodes (21): Base, ColorPalette, CubeConfig, Currency, Dashboard, DashboardAssignment, DataSource, DataType (+13 more)
...
Cohesion: 0.12
Nodes (11): ensure_user_exists(), get_current_user(), Ensure user exists in database, create if not, Ensure user exists in database, create if not, create_diagram_type(), list_diagram_types(), create_editor_tool(), _gen_id() (+3 more)
...
Cohesion: 0.13
Nodes (17): decrypt_value(), _get_fernet(), process_sensitive_fields(), Encryption utilities for sensitive data like API tokens., Get Fernet instance with key derived from settings., Decrypt a string value that was encrypted with encrypt_value.      Args:, Recursively encrypt/decrypt sensitive fields in a JSON-compatible structure., Recursively encrypt/decrypt sensitive fields in a JSON-compatible structure. (+9 more)
...
Cohesion: 0.19
Nodes (7): mock_get_current_user(), get_mock_db(), mock_get_current_user(), MockDS, get_jwks(), TokenData, verify_token()
...
Cohesion: 0.24
Nodes (10): areDatatypesCompatible(), canvasPos(), factNodeAt(), fieldAt(), handleFieldDropOnExisting(), keyFieldStartPos(), onCanvasDrop(), onGlobalMouseMove() (+2 more)

### Community 94 - "Community 94"
Cohesion: 0.22
Nodes (6): global_exception_handler(), test(), Validate configuration after initialization., Validate ODSConfig (internal hook for validation)., Validate ODSConfig (internal hook for validation)., type
...
Cohesion: 0.29
Nodes (6): Execute a single batch with retry logic.                  Args:             conf, Execute a single batch with retry logic.                  Args:             conf, Execute a single batch with retry logic.                  Args:             conf, Execute a single batch with retry logic.                  Args:             conf, Execute a single batch with retry logic.                  Args:             conf, Execute a single batch with retry logic.                  Args:             conf
...
Cohesion: 0.29
Nodes (7): addField(), deleteField(), handleFieldDrop(), refreshSelectedNode(), setKeyField(), updateField(), updateNodeName()
...
Cohesion: 0.33
Nodes (5): Execute upsert (INSERT ON CONFLICT DO UPDATE) for a batch.                  Args, Execute upsert (INSERT ON CONFLICT DO UPDATE) for a batch.                  Args, Execute upsert (INSERT ON CONFLICT DO UPDATE) for a batch.                  Args, Execute upsert (INSERT ON CONFLICT DO UPDATE) for a batch.                  Args, Execute upsert (INSERT ON CONFLICT DO UPDATE) for a batch.                  Args
...
- **Why does `DenoService` connect `Community 26` to `Community 7`, `Community 40`, `Community 13`, `Community 18`, `Community 52`, `Community 25`, `Community 27`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `type` connect `Community 94` to `Community 0`, `Community 26`, `Community 7`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `EmailPayload` connect `Community 52` to `Community 96`, `Community 1`, `Community 58`, `Community 13`, `Community 78`, `Community 79`, `Community 48`, `Community 47`, `Community 51`, `Community 26`, `Community 91`, `Community 92`?**
