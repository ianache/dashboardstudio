# Graph Report - dashboardstudio  (2026-06-02)

## Corpus Check
- 252 files · ~237,791 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2565 nodes · 2896 edges · 298 communities (227 shown, 71 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 286 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0c6e10e7`
...
Cohesion: 0.05
Nodes (34): global_exception_handler(), # NOTE: We manually add CORS headers here because unhandled exceptions can bypas, # NOTE: We manually add CORS headers here because unhandled exceptions can bypas, Handle EXEC_EMAIL signal by delegating to EmailExecutor.                  Args, Handle EXEC_EMAIL signal by delegating to EmailExecutor.                  Args, Handle EXEC_EMAIL signal by delegating to EmailExecutor.                  Args, Runs a flow in Deno and yields log messages.         Uses subprocess.run via ru, Runs a flow in Deno and yields log messages.         Uses subprocess.run via ru (+26 more)
...
Cohesion: 0.05
Nodes (34): assignedDSplitIds, current, data, date, defs, ds, dx, endpoint (+26 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (32): create_runner(), AI Analyst agent factory.  create_runner() builds a fresh LlmAgent and Runner, Factory: constructs a fresh LlmAgent + Runner for the requested model.      Uses, Factory: constructs a fresh LlmAgent + Runner for the requested model.      Us, calculate_cost(), chat(), ChatRequest, ensure_session() (+24 more)
...
Cohesion: 0.07
Nodes (23): run_migration(), BaseSettings, get_settings(), Settings, get_current_user_info(), get_user(), _keycloak_admin_token(), list_users() (+15 more)
...
Cohesion: 0.08
Nodes (23): ODSExecutor, Split records into batches of specified size.                  Args:, Core executor for PostgreSQL ODS operations.          This class provides method, Fetch all column names for the target table., Validate records for JSON serialization safety.                  Checks:, Fetch column names and their PostgreSQL data types., Align record keys with actual table columns:         - Filters out keys that do, Validate that the target schema and table exist.                  Args: (+15 more)
...
Cohesion: 0.09
Nodes (16): ODSConfig, Configuration for an ODS execution operation.          Attributes:         conne, Test statement timeout defaults per write mode., Test ODSConfig rejects empty connection_id., Test ODSConfig rejects empty schema., Test ODSConfig rejects empty table., Test ODSConfig requires identity_fields for upsert mode., Test WriteMode enum has expected values. (+8 more)
...
Cohesion: 0.1
Nodes (19): Enum, ExecutionHistory, NodeExecutionLogs, ODSError, ODSResult, ODS Executor Service - Core execution engine for PostgreSQL ODS operations.  Thi, Detailed error information for a failed batch or record.          Attributes:, Result of an ODS execution operation.          Attributes:         success: True (+11 more)
...
Cohesion: 0.1
Nodes (11): Tests for validation methods., Test batch size > 10000 is rejected., Test valid records pass validation., Test NaN values are rejected., Test positive Infinity values are rejected., Test negative Infinity values are rejected., Test large integers (> 2^53) are converted to strings., Test small integers remain as integers. (+3 more)
...
Cohesion: 0.16
Nodes (14): mock_get_current_user(), TokenData, create_data_source(), DataSourceCreate, DataSourceResponse, DataSourceUpdate, _encrypt_data_source_password(), _gen_id() (+6 more)
...
Cohesion: 0.15
Nodes (14): _decrypt_data_source_password(), get_data_source(), list_data_sources(), List all data sources., Search data sources by name or type, Get a single data source by ID., Test data source connection (admin/designer only), Decrypt password when returning to client. (+6 more)
...
Cohesion: 0.14
Nodes (12): flow_logs_websocket(), Streaming de logs de ejecución vía WebSocket., Streaming de logs de ejecución vía WebSocket., Streaming de logs de ejecución vía WebSocket., execute_llm_node(), Executes an LLM completion call using an OpenAI-compatible endpoint.     Includ, execute_ml_node(), Executes a scikit-learn model inference in an isolated subprocess.     Includes (+4 more)
...
Cohesion: 0.15
Nodes (12): Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., Ejecuta un flujo de integración en el runtime de Deno., run_integration_flow() (+4 more)
...
Cohesion: 0.18
Nodes (10): create_integration_flow(), _gen_id(), list_flow_executions(), list_integration_flows(), Lista las últimas ejecuciones de un flujo., Lista las últimas ejecuciones de un flujo., Lista las últimas ejecuciones de un flujo., Lista las últimas ejecuciones de un flujo. (+2 more)
...
Cohesion: 0.2
Nodes (7): lifespan(), init_db(), delete_integration_flow(), update_integration_flow(), init_scheduler(), schedule_flow(), unschedule_flow()
...
Cohesion: 0.47
Nodes (9): _err(), _execute_csv_local(), _execute_csv_s3(), _execute_http(), _execute_mysql(), _execute_postgres(), execute_source_node(), execute_sql() (+1 more)
...
Cohesion: 0.33
Nodes (6): normalize_filter(), query_data(), Fetches business data from CubeJS using the provided JSON query format., Fetches business data from CubeJS using the provided JSON query format., Fetches business data from CubeJS using the provided JSON query format., Fetches business data from CubeJS using the provided JSON query format.
...
Cohesion: 0.33
Nodes (5): Execute overwrite (TRUNCATE + INSERT) operation for a batch.                  On, Execute overwrite (TRUNCATE + INSERT) operation for a batch.                  On, Execute overwrite (TRUNCATE + INSERT) operation for a batch.                  On, Execute overwrite (TRUNCATE + INSERT) operation for a batch.                  On, Execute overwrite (TRUNCATE + INSERT) operation for a batch.                  On
...
- **Why does `DenoService` connect `Community 52` to `Community 1`, `Community 35`, `Community 11`, `Community 12`, `Community 46`, `Community 16`, `Community 21`, `Community 31`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `process_sensitive_fields()` connect `Community 84` to `Community 96`, `Community 64`, `Community 72`, `Community 73`, `Community 140`, `Community 52`, `Community 93`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `ODSExecutor` connect `Community 12` to `Community 1`, `Community 132`, `Community 133`, `Community 16`, `Community 21`, `Community 29`, `Community 30`, `Community 44`, `Community 45`, `Community 52`, `Community 63`, `Community 78`, `Community 103`, `Community 104`, `Community 105`, `Community 115`, `Community 116`, `Community 117`, `Community 118`, `Community 119`?**
