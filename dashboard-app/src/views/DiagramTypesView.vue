<template>
  <div class="min-h-[calc(100vh-64px)] p-8">
    ...
    <div v-if="deleteTarget" class="dt-overlay" @click.self="deleteTarget = null">
      <div class="dt-confirm-modal rounded-xl shadow-xl w-[420px] max-w-[95vw] overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b dt-confirm-border">
          <h3 class="text-base font-semibold dt-confirm-title">Eliminar Tipo de Diagrama</h3>
        </div>
        <div class="px-6 py-5">
          <p class="text-sm dt-confirm-title">¿Eliminar <strong>{{ deleteTarget.name }}</strong>? Las herramientas asociadas perderán este tipo de su lista.</p>
        </div>
        <div class="flex justify-end gap-3 px-6 py-4 border-t dt-confirm-footer">
...
.dt-h1  { font-family: 'Plus Jakarta Sans', sans-serif; font-size: 36px; font-weight: 700; color: var(--on-surface); line-height: 1.2; letter-spacing: -0.02em; }
...
.dt-card-name { font-size: 16px; font-weight: 700; color: var(--on-surface); font-family: 'Plus Jakarta Sans', sans-serif; }
.dt-card-desc { font-size: 13px; color: var(--on-surface-variant); flex: 1; }
.dt-card-meta { margin-top: 8px; }
.dt-tools-badge { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 500; color: var(--on-surface-variant); background: var(--surface-container); border-radius: 20px; padding: 3px 10px; }
.dt-card-footer { display: flex; align-items: center; justify-content: flex-end; gap: 6px; padding: 12px 16px; border-top: 1px solid var(--outline-variant); background: var(--surface-container); }
.dt-action { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border: none; background: var(--card-bg); border-radius: 7px; cursor: pointer; color: var(--on-surface-variant); transition: all 0.15s; box-shadow: 0 1px 2px rgba(0,0,0,0.06); }
...
.dt-new-card:hover { border-color: var(--primary); background: rgba(0,88,190,0.03); }
.dt-new-ico { width: 44px; height: 44px; border-radius: 50%; background: var(--surface-container); display: flex; align-items: center; justify-content: center; transition: background 0.2s; }
.dt-new-card:hover .dt-new-ico { background: var(--surface-container-high); }
.dt-new-lbl { font-size: 13px; font-weight: 600; color: var(--on-surface-variant); }
.dt-new-card:hover .dt-new-lbl { color: var(--primary); }
...
.dt-modal { background: var(--card-bg); width: 100%; max-width: 520px; border-radius: 12px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); display: flex; flex-direction: column; max-height: calc(100vh - 32px); overflow: hidden; }
.dt-modal-hdr { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 20px 24px; border-bottom: 1px solid var(--outline-variant); }
.dt-modal-title { font-size: 18px; font-weight: 700; color: var(--on-surface); font-family: 'Plus Jakarta Sans', sans-serif; }
.dt-modal-sub { font-size: 13px; color: var(--on-surface-variant); margin-top: 2px; }
.dt-close-btn { width: 30px; height: 30px; border: none; background: transparent; border-radius: 50%; cursor: pointer; color: var(--on-surface-variant); display: flex; align-items: center; justify-content: center; transition: all 0.15s; flex-shrink: 0; }
.dt-close-btn:hover { background: var(--surface-container); }
.dt-modal-body { padding: 24px; overflow-y: auto; flex: 1; }
.dt-modal-footer { display: flex; align-items: center; justify-content: flex-end; gap: 12px; padding: 16px 24px; background: var(--surface-container); border-top: 1px solid var(--outline-variant); }
.dt-field { margin-bottom: 16px; position: relative; }
.dt-label { display: block; font-size: 13px; font-weight: 500; color: var(--on-surface); margin-bottom: 6px; }
.dt-input, .dt-textarea { width: 100%; box-sizing: border-box; padding: 9px 13px; border: 1px solid var(--outline-variant); border-radius: 8px; font-size: 13px; background: var(--card-bg); color: var(--on-surface); outline: none; transition: all 0.15s; }
.dt-input:focus, .dt-textarea:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(37,99,235,0.1); }
.dt-input:disabled { background: var(--surface-container); color: var(--on-surface-variant); }
...
.dt-field-hint { font-size: 11px; color: var(--on-surface-variant); margin-top: 5px; }
.dt-form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.dt-color-row { display: flex; gap: 8px; align-items: center; }
.dt-color-swatch { width: 38px; height: 38px; border: 1px solid var(--outline-variant); border-radius: 8px; padding: 2px; cursor: pointer; flex-shrink: 0; }
...
.dt-confirm-modal { background: var(--card-bg); border: 1px solid var(--outline-variant); }
.dt-confirm-border { border-color: var(--outline-variant); }
.dt-confirm-title { color: var(--on-surface); }
.dt-confirm-footer { background: var(--surface-container); border-color: var(--outline-variant); }
</style>
