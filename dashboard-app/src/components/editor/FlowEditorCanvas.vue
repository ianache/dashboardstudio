  ...
  <div class="fec-root" :class="{ 'fec-root--readonly': readOnly }" @mousemove="onGlobalMousemove" @mouseup="onGlobalMouseup">
    ...
    <aside v-if="!readOnly" class="fec-left" :class="{ 'fec-left--collapsed': leftCollapsed }">
      ...
      <div class="fec-left-inner">
        <div v-if="!leftCollapsed" class="fec-left-head">
          <span class="msi" style="font-size:15px;color:var(--primary)">widgets</span>
        ...
        <template v-for="cat in toolsByCategory" :key="cat.key">
          <div class="fec-cat">
            <div class="fec-cat-hdr" @click="!leftCollapsed && toggleCat(cat.key)" :title="cat.label">
              ...
              <span v-if="!leftCollapsed" class="msi fec-cat-arr" :class="{ 'fec-cat-arr--open': openCats[cat.key] }" style="font-size:15px;color:var(--on-surface-variant)">expand_more</span>
            </div>
        ...
        <div v-if="toolsByCategory.length === 0 && !leftCollapsed" class="fec-no-tools">
          <span class="msi" style="font-size:28px;color:var(--border)">construction</span>
    ...
    <main
      ...
      <div
        ...
        <span class="msi fec-fbar-handle" style="font-size:17px;color:var(--on-surface-variant);cursor:grab">drag_indicator</span>
      ...
      <div v-if="isDragOver" class="fec-drop-hint">
        <span class="msi" style="font-size:32px;color:var(--primary)">add_circle</span>
      ...
      <div
        ...
        <div
          ...
          :style="{
            ...
            background: note.props.color || 'var(--warning-container)',
            borderColor: darkenColor(note.props.color || 'var(--warning-container)'),
          ...
          <div class="fec-note-body">
            ...
            <div v-if="(selectedNote?.id === note.id || editingNoteId === note.id) && !readOnly" class="fec-note-toolbar" @mousedown.stop @click.stop>
              <div class="fec-note-palette">
                <button v-for="c in ['color-mix(in srgb, var(--warning) 10%, transparent)', 'color-mix(in srgb, var(--primary) 10%, transparent)', 'color-mix(in srgb, var(--success) 10%, transparent)', 'color-mix(in srgb, var(--error) 10%, transparent)', 'var(--surface-container-low)']" 
        ...
        <svg :width="CANVAS_W" :height="CANVAS_H" style="position:absolute;top:0;left:0;overflow:visible">
          ...
          <path
            ...
            :stroke="selectedConn === conn.id || hoveredConn === conn.id ? 'var(--primary)' : ((readOnly ? nodeLogsMap[conn.from]?.status : nodeExecStatus[conn.from]) === 'success' ? 'var(--success)' : 'var(--on-surface-variant)')"
      ...
      <div v-if="!rightCollapsed" class="fec-right-inner">
        ...
        <template v-if="!selectedNode && rightTab === 'props'">
          <div class="fec-props-hdr">
            <span class="msi" style="font-size:20px;color:var(--primary)">settings</span>
            <div>
        ...
        <template v-if="!selectedNode && rightTab === 'variables'">
          <div class="fec-props-hdr">
            <span class="msi" style="font-size:20px;color:var(--primary)">variable_insert</span>
            <div>
        ...
        <template v-else-if="selectedNote">
          <div class="fec-props-hdr">
            <div class="fec-props-node-ico" style="background:var(--warning-container);color:var(--warning)">
          ...
          <div class="fec-prop-g">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <button class="fec-note-tbtn" style="border:1px solid var(--border);background:var(--surface);" @click="changeNoteFontSize(selectedNote, -1)"><span class="msi" style="font-size:14px">remove</span></button>
              <span class="fec-note-tsize">{{ parseInt(selectedNote.props.fontSize || 13) }}px</span>
              <button class="fec-note-tbtn" style="border:1px solid var(--border);background:var(--surface);" @click="changeNoteFontSize(selectedNote, 1)"><span class="msi" style="font-size:14px">add</span></button>
            </div>
          </div>
        ...
        <template v-else-if="selectedNode">
          ...
          <div v-if="selectedNode.toolType === 'pickle_model' && selectedNode.props.model_id" class="fec-ml-info">
            ...
            <div v-if="selectedModel" class="fec-ml-details">
              ...
              <div class="fec-arch-warning" style="margin-top:12px;background:color-mix(in srgb, var(--primary) 10%, transparent);border-color:color-mix(in srgb, var(--primary) 20%, transparent);border-left-color:var(--primary);color:var(--on-surface)">
                <span class="msi" style="color:var(--primary)">info</span>
...
function getCatColor(cat) { return CAT_META[cat?.toLowerCase()]?.color || 'var(--on-surface-variant)' }
function getCatBg(cat)    { return CAT_META[cat?.toLowerCase()]    || 'var(--surface-container-low)' }
...
const toolsByCategory = computed(() => {
  ...
  return Object.entries(map).map(([cat, items]) => ({
    ...
    color: CAT_META[cat]?.color || 'var(--on-surface-variant)',
    bg:    CAT_META[cat]?.bg    || 'var(--surface-container-low)',
...
.fec-toggle {
  ...
  color: var(--on-surface-variant); box-shadow: var(--shadow); transition: all 0.15s;
}
.fec-toggle:hover { background: color-mix(in srgb, var(--primary) 10%, transparent); color: var(--primary); }
...
.fec-left-inner::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }

.fec-left-head {
  display: flex; align-items: center; gap: 7px;
  padding: 10px 14px 8px;
}
.fec-panel-label { font-size: 11px; font-weight: 700; color: var(--on-surface-variant); text-transform: uppercase; letter-spacing: 0.07em; flex: 1; }
.fec-diagram-tag {
  font-size: 9px; font-weight: 600; color: var(--primary);
  background: color-mix(in srgb, var(--primary) 10%, transparent); border-radius: 4px; padding: 2px 6px; white-space: nowrap;
}
...
.fec-cat-hdr:hover { background: var(--surface-container-low); }
.fec-cat-hdr-l { display: flex; align-items: center; gap: 8px; }
.fec-cat-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
.fec-cat-name { font-size: 11px; font-weight: 600; color: var(--on-surface); }
...
.fec-comp-item:hover { background: var(--surface-container-low); }
...
.fec-comp-name { display: block; font-size: 12px; font-weight: 500; color: var(--on-surface); line-height: 1.3; }
.fec-comp-sub  { display: block; font-size: 10px; color: var(--on-surface-variant); }
.fec-no-tools  { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 32px 16px; text-align: center; font-size: 12px; color: var(--on-surface-variant); }
...
.fec-float-bar {
  ...
  padding: 4px 8px; box-shadow: var(--shadow-md); z-index: 20;
}
.fec-fbar-handle { padding: 0 4px; }
.fec-fsep { width: 1px; height: 18px; background: var(--border); margin: 0 4px; }
.fec-tbtn {
  width: 28px; height: 28px; display: flex; align-items: center; justify-content: center;
  border: none; background: transparent; border-radius: 6px; cursor: pointer;
  color: var(--on-surface-variant); transition: all 0.12s;
}
.fec-tbtn:hover { background: var(--surface-container-low); color: var(--primary); }
.fec-tbtn--on { background: color-mix(in srgb, var(--primary) 10%, transparent); color: var(--primary); }
.fec-zoom-pct { font-size: 11px; font-weight: 600; color: var(--on-surface-variant); min-width: 34px; text-align: center; }

/* Drop hint */
.fec-drop-hint {
  position: absolute; inset: 0; z-index: 15; pointer-events: none;
  display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px;
  background: color-mix(in srgb, var(--primary) 5%, transparent); border: 2px dashed var(--primary); border-radius: 12px; margin: 12px;
...
.fec-node:hover { box-shadow: var(--shadow-md); border-color: var(--on-surface-variant); }
.fec-node--sel  { border-color: var(--primary) !important; box-shadow: 0 0 0 3px color-mix(in srgb, var(--primary) 15%, transparent); }

/* Execution visual states */
.fec-node--executing { border-color: var(--success) !important; border-width: 3px !important; box-shadow: 0 0 15px color-mix(in srgb, var(--success) 30%, transparent); }
.fec-node--success   { border-color: var(--success) !important; }
.fec-node--error     { border-color: var(--error) !important; }

/* Status Badges */
.fec-node-badge {
  position: absolute; top: -10px; right: -10px; width: 22px; height: 22px;
  border-radius: 50%; background: var(--surface); z-index: 5;
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 2px 6px rgba(0,0,0,0.15);
}
.fec-node-badge .msi { font-size: 18px; }
.fec-node-badge--success { color: var(--success); }
.fec-node-badge--error   { color: var(--error); }

.fec-node-badge--left {
  position: absolute; top: -10px; left: -10px; min-width: 22px; height: 22px;
  padding: 0 6px; background: var(--surface); color: var(--on-surface-variant); border-radius: 11px;
  font-size: 10px; font-weight: 800; display: flex; align-items: center; justify-content: center;
  box-shadow: var(--shadow); border: 1.5px solid var(--border); z-index: 5;
}

/* Tooltip (Ficha) */
.fec-node-tooltip {
  position: absolute; bottom: calc(100% + 12px); left: 50%; transform: translateX(-50%);
  width: 200px; background: var(--surface); border: 1px solid var(--border); border-radius: 12px;
...
.fec-tooltip-hdr { font-size: 9px; font-weight: 800; color: var(--on-surface-variant); text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 6px; }
.fec-tooltip-title { font-size: 13px; font-weight: 700; color: var(--on-surface); margin-bottom: 8px; }
...
.fec-status-badge.success { background: color-mix(in srgb, var(--success) 10%, transparent); color: var(--success); }
.fec-status-badge.error   { background: color-mix(in srgb, var(--error) 10%, transparent); color: var(--error); }
...
.fec-tooltip-row .msi.start { color: var(--success); }
.fec-tooltip-row .msi.end { color: var(--error); }

.fec-tooltip-label { font-size: 9px; font-weight: 700; color: var(--on-surface-variant); margin: 0; }
.fec-tooltip-val   { font-size: 11px; font-weight: 600; color: var(--on-surface); margin: 1px 0 0; }

.fec-tooltip-footer {
  display: flex; align-items: center; gap: 6px; padding-top: 10px;
  border-top: 1px solid var(--surface-container-low); font-size: 10px; color: var(--on-surface-variant);
}
.fec-tooltip-footer .msi { font-size: 15px; color: var(--primary); }
...
.fec-node-hdr-ico {
  ...
  background: var(--nc); color: var(--on-primary);
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
}
.fec-node-lbl { font-size: 12px; font-weight: 600; color: var(--on-surface); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; }
.fec-node-bdy  { padding: 8px 10px; display: flex; flex-direction: column; gap: 3px; }
.fec-node-tag  { font-size: 10px; font-weight: 500; color: var(--on-surface-variant); background: var(--surface-container-low); border-radius: 4px; padding: 1px 5px; display: inline-block; max-width: fit-content; }
.fec-node-meta { font-size: 10px; color: var(--on-surface-variant); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

/* Ports */
.fec-port {
  position: absolute; width: 12px; height: 12px; border-radius: 50%;
  background: var(--surface); border: 2px solid var(--on-surface-variant); top: 50%; transform: translateY(-50%);
  cursor: crosshair; transition: border-color 0.12s, transform 0.12s; z-index: 2;
}
.fec-port:hover { border-color: var(--primary); transform: translateY(-50%) scale(1.3); }
...
.fec-port--true  { border-color: var(--success) !important; }
.fec-port--true:hover { background: color-mix(in srgb, var(--success) 10%, transparent); }
.fec-port--false { border-color: var(--error) !important; }
.fec-port--false:hover { background: color-mix(in srgb, var(--error) 10%, transparent); }

.fec-port-label {
  position: absolute; right: 16px; top: 50%; transform: translateY(-50%);
  font-size: 9px; font-weight: 800; color: var(--on-surface-variant); pointer-events: none;
}
.fec-port--true .fec-port-label { color: var(--success); }
.fec-port--false .fec-port-label { color: var(--error); }
...
.fec-conn-lbl { font-size: 12px; color: var(--on-surface-variant); }
.fec-conn-del {
  display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px;
  border-radius: 6px; font-size: 12px; font-weight: 500;
  background: color-mix(in srgb, var(--error) 10%, transparent); color: var(--error); border: 1px solid color-mix(in srgb, var(--error) 20%, transparent); cursor: pointer; transition: all 0.12s;
}
.fec-conn-del:hover { background: color-mix(in srgb, var(--error) 20%, transparent); }

/* Connection visual states */
.fec-conn { transition: stroke 0.3s, stroke-width 0.3s; }
.fec-conn--active { stroke: var(--success) !important; stroke-width: 3 !important; }
...
.fec-resizer:hover { background: color-mix(in srgb, var(--primary) 20%, transparent); }
...
.fec-right-inner::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }

/* Properties */
.fec-props-hdr { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 16px; }
.fec-props-title { font-size: 13px; font-weight: 700; color: var(--on-surface); font-family: var(--font-heading); }
.fec-props-sub { font-size: 11px; color: var(--on-surface-variant); margin-top: 1px; }
...
.fec-close-btn {
  ...
  color: var(--on-surface-variant); display: flex; align-items: center; justify-content: center; transition: all 0.12s;
}
.fec-close-btn:hover { background: var(--surface-container-low); color: var(--on-surface); }

/* Form fields */
.fec-prop-g  { margin-bottom: 12px; }
.fec-prop-l  { display: block; font-size: 10px; font-weight: 700; color: var(--on-surface-variant); margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.05em; }
.fec-prop-i, .fec-prop-ta, .fec-prop-sel {
  width: 100%; box-sizing: border-box; padding: 7px 10px; font-size: 12px;
  border: 1px solid var(--border); border-radius: 7px; background: var(--surface); color: var(--on-surface); outline: none;
  transition: border-color 0.15s, box-shadow 0.15s; font-family: inherit;
}
.fec-prop-i::placeholder, .fec-prop-ta::placeholder { color: var(--on-surface-variant); opacity: 0.6; }
...
.fec-sel-arr  { position: absolute; right: 7px; top: 50%; transform: translateY(-50%); color: var(--on-surface-variant); pointer-events: none; }

.fec-divider {
  display: flex; align-items: center; gap: 8px; margin: 4px 0 12px;
  font-size: 9px; font-weight: 700; color: var(--on-surface-variant); text-transform: uppercase; letter-spacing: 0.08em;
}
.fec-divider::before, .fec-divider::after { content: ''; flex: 1; height: 1px; background: var(--surface-container-low); }
.fec-optional-tag { font-weight: 400; color: var(--on-surface-variant); opacity: 0.6; text-transform: none; letter-spacing: 0; }

.fec-flow-stats { margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--surface-container-low); display: flex; flex-direction: column; gap: 7px; }
.fec-stat-row  { display: flex; align-items: center; gap: 7px; font-size: 12px; color: var(--on-surface-variant); }

.fec-node-del-wrap { margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--surface-container-low); }
.fec-node-del-btn {
  display: flex; align-items: center; gap: 6px; width: 100%; padding: 7px 12px;
  background: color-mix(in srgb, var(--error) 10%, transparent); color: var(--error); border: 1px solid color-mix(in srgb, var(--error) 20%, transparent); border-radius: 8px;
  font-size: 12px; font-weight: 500; cursor: pointer; transition: all 0.12s;
}
.fec-node-del-btn:hover { background: color-mix(in srgb, var(--error) 20%, transparent); }

/* Tabs */
.fec-tabs { display: flex; gap: 4px; border-bottom: 1px solid var(--surface-container-low); margin-bottom: 16px; padding-bottom: 8px; }
.fec-tab {
  flex: 1; display: flex; align-items: center; justify-content: center; gap: 6px;
  padding: 6px 4px; border: none; background: transparent; cursor: pointer;
  font-size: 11px; font-weight: 600; color: var(--on-surface-variant); border-radius: 6px; transition: all 0.15s;
}
.fec-tab:hover { background: var(--surface-container-low); color: var(--on-surface); }
.fec-tab.active { background: color-mix(in srgb, var(--primary) 10%, transparent); color: var(--primary); }
...
.fec-history-item:hover { border-color: var(--primary); box-shadow: var(--shadow); }
...
.fec-hi-status--success { background: var(--success); box-shadow: 0 0 6px color-mix(in srgb, var(--success) 30%, transparent); }
.fec-hi-status--error { background: var(--error); }
.fec-hi-date { font-size: 11px; font-weight: 600; color: var(--on-surface); flex: 1; }
.fec-hi-duration { font-size: 10px; color: var(--on-surface-variant); }
.fec-hi-footer { display: flex; align-items: center; justify-content: space-between; margin-top: 4px; }
.fec-hi-id { font-size: 10px; color: var(--on-surface-variant); opacity: 0.6; font-family: monospace; }
.fec-history-loading, .fec-history-empty { padding: 32px 16px; text-align: center; font-size: 12px; color: var(--on-surface-variant); }
...
.fec-run-btn:hover:not(:disabled) { background: var(--primary-container); color: var(--on-primary-container); transform: translateY(-1px); box-shadow: var(--shadow-md); }
.fec-run-btn:disabled { background: var(--on-surface-variant); cursor: not-allowed; opacity: 0.7; }
...
.fec-resizer-v:hover { background: color-mix(in srgb, var(--primary) 20%, transparent); }

/* Connection binding helpers */
.fec-conn-hint   { font-size: 11px; color: var(--on-surface-variant); margin-top: 4px; }
.fec-conn-spin   { margin-left: 6px; vertical-align: middle; }
.fec-conn-filled { font-size: 11px; color: var(--success); margin-top: 4px; display: flex; align-items: center; gap: 4px; }

/* ── Notes (Annotations) ───────────────────────────────────────── */
.fec-node--annotations {
  width: 240px; min-height: 120px; padding: 0;
  background: var(--warning-container); border-color: var(--warning);
...
.fec-note-tsep { width: 1px; height: 16px; background: var(--border); margin: 0 4px; }
.fec-note-tbtn {
  width: 24px; height: 24px; display: flex; align-items: center; justify-content: center;
  border: none; background: transparent; cursor: pointer; color: var(--on-surface-variant); border-radius: 4px;
}
.fec-note-tbtn:hover { background: var(--surface-container-low); color: var(--primary); }
.fec-note-tbtn--del:hover { background: color-mix(in srgb, var(--error) 10%, transparent); color: var(--error); }
.fec-note-tsize { font-size: 11px; font-weight: 700; color: var(--on-surface-variant); min-width: 16px; text-align: center; }

.fec-note-ta {
  width: 100%; height: 100%; min-height: 100px;
  border: none; background: transparent; resize: none;
  font-family: inherit; font-size: 13px; color: var(--on-surface); outline: none;
}
.fec-note-content {
  flex: 1; font-size: 13px; color: var(--on-surface); line-height: 1.5;
  overflow-y: auto; overflow-x: hidden;
}

/* Markdown Styling inside notes */
.fec-note-content :deep(h1) { font-size: 1.25rem; font-weight: 700; margin: 0 0 0.5rem; color: var(--on-surface); }
.fec-note-content :deep(h2) { font-size: 1.1rem; font-weight: 700; margin: 0.75rem 0 0.4rem; color: var(--on-surface); }
...
.fec-note-resizer {
  ...
  display: flex; align-items: center; justify-content: center;
  color: var(--on-surface-variant); cursor: nwse-resize; z-index: 10;
  transition: color 0.1s;
}
.fec-note-resizer:hover { color: var(--primary); }
...
.fec-var-item { 
  ...
  padding: 10px; border: 1px solid var(--surface-container-low); border-radius: 10px; background: var(--surface);
}
...
.fec-var-del:hover { background: color-mix(in srgb, var(--error) 10%, transparent); color: var(--error); }
...
.fec-add-var-btn {
  ...
  color: var(--on-surface-variant); font-size: 12px; font-weight: 600; cursor: pointer;
  transition: all 0.2s;
}
.fec-add-var-btn:hover { background: var(--surface-container-low); border-color: var(--primary); color: var(--primary); }
...
/* ── ML Info Styles ────────────────────────────────────────────── */
.fec-ml-info { margin-top: 16px; padding: 12px; background: var(--surface-container-low); border: 1px solid var(--border); border-radius: 10px; }
.fec-ml-details { display: flex; flex-direction: column; gap: 10px; }
.fec-ml-row { display: flex; align-items: center; gap: 6px; font-size: 11px; font-weight: 600; color: var(--on-surface-variant); }
.fec-ml-row .msi { color: var(--primary); }
.fec-ml-features-wrap { display: flex; flex-direction: column; gap: 6px; }
.fec-ml-feature-tags { display: flex; flex-wrap: wrap; gap: 4px; }
.fec-ml-tag { font-size: 10px; font-family: monospace; background: var(--surface); border: 1px solid var(--border); padding: 1px 6px; border-radius: 4px; color: var(--primary); }
.fec-ml-loading { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; font-size: 11px; color: var(--on-surface-variant); }
...
.fec-preview-btn {
  ...
  padding: 8px; background: var(--surface); border: 1px solid var(--primary);
  color: var(--primary); border-radius: 8px; font-size: 12px; font-weight: 600;
